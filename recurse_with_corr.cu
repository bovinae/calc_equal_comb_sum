...
__host__ __device__
void printVector(int* lhs, int lhsSize, my_vector<int>& rhs, int* sel) {
  if (lhsSize == 0 || rhs.len == 0) return;
  for (int i = 0; i < lhsSize; i++) {
    if (i == 0) printf("%d", int(sel[lhs[i]]));
    else printf(" + %d", int(sel[lhs[i]]));
  }
  printf(" = ");
  for (int i = 0; i < rhs.len; i++) {
    if (i == 0) printf("%d", int(sel[rhs.mv[i]]));
    else printf(" + %d", int(sel[rhs.mv[i]]));
  }
  printf("\n");
}
...
__device__
double continueDegree(my_vector<int>& rhs) {
  int total = 0;
  for (int i = 0; i < rhs.len-1 && rhs.len > 1; i++) {
    if (rhs.mv[i]-1 == rhs.mv[i+1]) {
      // printf("%d %d\n", lhs[i], lhs[i+1]);
      total++;
    }
  }
  double ret = double(total+1) / double(rhs.len);
  if (ret < 0.5) printf("%f\n", ret);
  return ret;
}

__device__
void recurse(int thd, DataWithPos* d, double **mat, int row, double& target, int* lhs, int lhsSize, char* lhsMap, my_vector<int>& rhs, int begin, int& recurseDepth, int* sel) {
  // if (recurseDepth >= 4 && rhs.mv[0] <= 8000) return ;
  ...
  for (int i = begin; i >= 0; i--) {
    ...
    if (lhsMap[i] == 1) continue;
    rhs.mv[rhs.len] = i;
    rhs.len++;
    target -= d[i].data;
    recurse(thd, d, mat, row, target, lhs, lhsSize, lhsMap, rhs, i-1, recurseDepth, sel);
    // if (recurseDepth < 4 || rhs.mv[0] > 8000) {
      target += d[i].data;
      rhs.len--;
    // }
  }
...
__global__ 
void recurse_kernel(DataWithPos* d, double **mat, int row, double target, int* lhs, int lhsSize, char* lhsMap, my_vector<int>* rhs, int begin, int* sel)
{
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  // printf("thd: %d\n", i);
  if (i < 0 || i > begin) return;
  // if (isInLhs(lhs, lhsSize, i)) return;
  if (lhsMap[i] == 1) return;
  ...
  // do {
    recurseDepth = 0;
    recurse(i, d, mat, row, recurseTarget, lhs, lhsSize, lhsMap, rhs[i], rhs[i].mv[rhs[i].len-1] - 1, recurseDepth, sel);
    // printf("recurseDepth: %d\n", recurseDepth);
  // } while (recurseDepth >= 4 && rhs[i].mv[0] <= 8000);
  rhs[i].len--;
}

void parallel(DataWithPos* d, double **mat, int row, double target, int* lhs, int lhsSize, char* lhsMap, int begin, my_vector<int> *many_rhs, int* sel) {
  dim3 blockSize(256);
  dim3 gridSize((begin + 1 + blockSize.x - 1) / blockSize.x);
  recurse_kernel<<<gridSize, blockSize>>>(d, mat, row, target, lhs, lhsSize, lhsMap, many_rhs, begin, sel);
  cudaDeviceSynchronize();
}

bool hasContinue(vector<int>& lhs) {
  for (int i = 0; i < lhs.size()-1 && lhs.size() > 1; i++) {
    if (lhs[i]-1 == lhs[i+1]) {
      // printf("%d %d\n", lhs[i], lhs[i+1]);
      return true;
    }
...
void combination(DataWithPos* d, double **mat, int row, int col, int lhsNum, vector<int>& lhs, int* lhsDev, char* lhsMap, my_vector<int> *many_rhs, int* sel) {
  if (lhsNum == 0) {
    // printCudaMem();
    ...
    parallel(d, mat, row, target, lhsDev, lhs.size(), lhsMap, lhs[0]-1, many_rhs, sel);
    return ;
  }
  ...
  for (int i = begin; i >= end; i--) { // todo: begin 4215
    ...
    lhs.push_back(i);
    combination(d, mat, row, col, lhsNum-1, lhs, lhsDev, lhsMap, many_rhs, sel);
    lhs.pop_back();
  }
}

void find(DataWithPos* d, double **mat, int row, int col, vector<int>& selVec) {
  struct sysinfo si;
  sysinfo(&si);
  ...
  for (int lhsNum = 1; lhsNum <= col/2; lhsNum++) {
    ...
    thread t([=, &thread_num, &selVec]{
      printf("processing lhsNum: %d\n", lhsNum);
      int* sel;
      cudaMallocManagedWrap(&sel, selVec.size() * sizeof(int));
      int *lhsDev;
      ...
      vector<int> lhs;
      combination(d, mat, row, col, lhsNum, lhs, lhsDev, lhsMap, many_rhs, sel);
      cudaFree(many_rhs);
      delete []tmp;
      cudaFree(lhsMap);
      cudaFree(lhsDev);
...
int main(int argc, char** argv)
{
  ...
  readFile(fileName.c_str(), mat, selVec);
  ...
  find(dataWithPos, mat, row, col, selVec);
